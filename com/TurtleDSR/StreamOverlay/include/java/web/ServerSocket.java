package com.TurtleDSR.StreamOverlay.include.java.web;
  ...
  static class MainHandler implements HttpHandler {
    ...
    public void handle(HttpExchange exchange) throws IOException { //sends css and js files from static folder
      ...
      if(filePathArr.length == 2) { //root path
        ...
        response = Files.readAllBytes(Paths.get("com/TurtleDSR/StreamOverlay/include/web" + fileName + ".html"));
        ...
        if(!fileNameArr[0].equals("main")) {
          ...
          String oDef = "\nlet o = new " + refererId[0] + "(\"" + id + "\")";

          response = concatenateByteArrays(Files.readAllBytes(Paths.get("com/TurtleDSR/StreamOverlay/include/web" + exchange.getRequestURI().getPath())), oDef.getBytes());
        } else {
          response = Files.readAllBytes(Paths.get("com/TurtleDSR/StreamOverlay/include/web" + exchange.getRequestURI().getPath()));
        }  
      } else {
        response = Files.readAllBytes(Paths.get("com/TurtleDSR/StreamOverlay/include/web" + exchange.getRequestURI().getPath()));
      }
